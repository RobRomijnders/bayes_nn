...
for mutilation, var_name, _, _ in conf.experiments:
    ...
    for num_experiment in range(num_experiments):
        ...
        for num_row in range(num_rows):
            ...
            axarr[num_row, 0].set_title('Entropy %5.3f' % risks[num_experiment, 0, batch_count], color=color)

            axarr[num_row, 1].imshow(np.ones((28, 28)) * risks[num_experiment, 0, batch_count], cmap='coolwarm', vmin=0.0, vmax=1.0)
            axarr[num_row, 1].set_title(f'Epistemic uncertainty{risks[num_experiment, 0, batch_count]:7.2f}')
            axarr[num_row, 2].imshow(np.ones((28, 28)) * risks[num_experiment, 1, batch_count], cmap='coolwarm', vmin=0.0, vmax=0.5)
            axarr[num_row, 2].set_title(f'Aleatoric uncertainty{risks[num_experiment, 1, batch_count]:7.3f}')
            batch_count += 1
